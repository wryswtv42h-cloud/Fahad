...
// أقسام الميزات الإضافية (السينما، الألعاب، التذاكر، التقديمات)
function renderCinema(list){
  content.className="grid";
  content.innerHTML=list.length?list.map(m=>`<article class="card" style="flex-direction:column; align-items:flex-start; gap:10px;"><h3>🎬 ${esc(m.title)}</h3><p class="muted">التصنيف: ${esc(m.category)} | المدة: ${esc(m.duration)}</p><button class="primary" style="width:100%; margin-top:5px;" onclick="alert('جاري تشغيل الفلم عبر البوت في الروم الصوتي!')">تشغيل السينما</button></article>`).join(""):`<p class="muted">لا توجد أفلام مضافة حالياً.</p>`;
}

function renderGames(list){
  content.className="grid";
  content.innerHTML=`<div style="grid-column: 1/-1; text-align:center; margin-bottom:15px;"><button class="primary" onclick="alert('تم فتح طاولة لعب جديدة!')">إنشاء طاولة لعب 🎮</button></div>` + (list.length?list.map(g=>`<article class="card"><div><h3>🎮 ${esc(g.gameName)}</h3><p class="muted">المضيف: ${esc(g.host)}</p></div></article>`).join(""):`<p class="muted" style="grid-column:1/-1; text-align:center;">لا توجد طاولات ألعاب نشطة حالياً.</p>`);
}

function renderTickets(list){
  content.className="grid";
  content.innerHTML=`<div style="grid-column: 1/-1; text-align:center; margin-bottom:15px;"><button class="primary" onclick="alert('تم فتح تذكرة دعم فني جديدة!')">فتح تذكرة جديدة 🎫</button></div>` + (list.length?list.map(t=>`<article class="card"><div><h3>🎫 ${esc(t.subject)}</h3><p class="muted">بواسطة: ${esc(t.username)} (${esc(t.status)})</p></div></article>`).join(""):`<p class="muted" style="grid-column:1/-1; text-align:center;">لا توجد تذاكر مفتوحة حالياً.</p>`);
}

function renderApplications(list){
  content.className="grid";
  content.innerHTML=`<div style="grid-column: 1/-1; text-align:center; margin-bottom:15px;"><button class="primary" onclick="alert('تم إرسال طلب التقديم بنجاح!')">تقديم طلب جديد 📋</button></div>` + (list.length?list.map(a=>`<article class="card"><div><h3>📋 طلب من: ${esc(a.username)}</h3><p class="muted">${esc(a.details)}</p></div></article>`).join(""):`<p class="muted" style="grid-column:1/-1; text-align:center;">لا توجد تقديمات إدارية حالياً.</p>`);
}
...
async function refresh(){try{const [sr,rr]=await Promise.all([fetch("/api/public/server"),fetch("/api/public/roles")]);const s=await sr.json(),rd=await rr.json();$("#server-name").textContent=s.name\vert{}\vert{}"MLD";$("#server-count").textContent=num(s.memberCount);roles=rd.roles||[];if(s.invite){$("#invite").href=s.invite;$("#invite-mobile").href=s.invite}else{$("#invite").style.display="none";$("#invite-mobile").style.display="none"}
if(view==="members"&&!search.value){const d=await fetch("/api/public/members").then(r=>r.json());all=d.members||[];renderMembers(all);setStatus(`${num(all.length)} عضو متصل`)}
else if(view==="roles")renderRoles();
else if(view==="top")renderTop(await fetch("/api/public/top").then(r=>r.json()));
else if(view==="cinema"){const d=await fetch("/api/public/movies").then(r=>r.json());renderCinema(d.movies||[]);setStatus("السينما جاهزة")}
else if(view==="games"){const d=await fetch("/api/public/games/sessions").then(r=>r.json());renderGames(d.sessions||[]);setStatus("الألعاب جاهزة")}
else if(view==="tickets"){const d=await fetch("/api/public/tickets").then(r=>r.json());renderTickets(d.tickets||[]);setStatus("التذاكر جاهزة")}
else if(view==="applications"){const d=await fetch("/api/public/applications").then(r=>r.json());renderApplications(d.applications||[]);setStatus("التقديمات جاهزة")}
...
async function change(v){
  view=v; mobile.classList.remove("open");
  if(v==="message")return messageView();
  searchWrap.style.display=v==="members"?"flex":"none";
  title.textContent=v==="members"?"أعضاء المجتمع":v==="roles"?"الرتب القيادية الست":v==="cinema"?"السينما 🎬":v==="games"?"الألعاب الجماعية 🎮":v==="tickets"?"نظام التذاكر 🎫":v==="applications"?"التقديمات الإدارية 📋":"لوحة TOP";
  
  if(v==="members"||v==="roles"||v==="cinema"||v==="games"||v==="tickets"||v==="applications")return refresh();
