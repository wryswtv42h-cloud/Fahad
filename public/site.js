...
function topSec(t, list, k, l) {
  return `
    <section class="top-section">
      ...
      ${
        list.map((m, i) => `
          <article class="top-card" data-member="${esc(m.id)}">
...
function renderRoles() {
  ...
  content.innerHTML = roles.map(r => `
    <article class="role-card" data-role="${esc(r.id)}">
      ...
      <div class="roles">
        ${
          (r.permissions || [])
            .slice(0, 4)
            .map(p =>
              `<span class="permission">${esc(p)}</span>`
            )
            .join("")
          ||
...
async function refresh() {
  try {
    ...
    if (s.invite) {
      ...
      $("#invite-mobile").style.display = "none";
    }
