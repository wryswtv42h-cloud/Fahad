...
// قواعد البيانات الجديدة للميزات المضافة (السينما، الألعاب، التذاكر، التقديمات)
const moviesLibrary = [
  { id: 1, title: "فلم أكشن ومغامرات", duration: "ساعتان", category: "أكشن" },
  { id: 2, title: "فلم دراما وتشويق", duration: "ساعة ونصف", category: "دراما" }
];
const gameSessions = [];
const ticketsList = [];
...
// ----------------- الأكواد والمسارات الأصلية (بدون تغيير) -----------------
...
// ----------------- الإضافات الجديدة المطلوبة (السينما، الألعاب، التذاكر، التقديمات) -----------------

// 1. نظام السينما والأفلام
app.get("/api/public/movies", (req, res) => {
  res.json({ movies: moviesLibrary });
});

app.post("/api/public/cinema/play", async (req, res) => {
  const { movieId, channelId } = req.body;
  const movie = moviesLibrary.find(m => m.id === Number(movieId));
  if (!movie) return res.status(404).json({ error: "الفلم غير موجود" });

  try {
    const guild = await getGuild();
    const channel = guild.channels.cache.get(channelId);
    if (!channel) return res.status(404).json({ error: "الروم الصوتي غير موجود" });
    
    // البوت جاهز لدخول الروم وتشغيل السينما
    res.json({ ok: true, message: `جاري تشغيل فلم (${movie.title}) في الروم (${channel.name}) والبوت في الخدمة!` });
  } catch (err) {
    res.status(500).json({ error: "حدث خطأ أثناء تشغيل السينما" });
  }
});

// 2. نظام الألعاب والقروبات (بالوت، أونو، لودو، جاكارو، وغيرها)
app.get("/api/public/games/sessions", (req, res) => {
  res.json({ sessions: gameSessions });
});

app.post("/api/public/games/create", (req, res) => {
  const { gameName, maxPlayers, hostName } = req.body;
  if (!gameName) return res.status(400).json({ error: "اسم اللعبة مطلوب" });
  ...
  const newSession = {
    ...
    host: hostName || "عضو",
    playersCount: 1,
    status: "جاري الانتظار / متفرجين متاحين"
  };
...
app.post("/api/public/tickets/create", (req, res) => {
  const { subject, username } = req.body;
  if (!subject) return res.status(400).json({ error: "عنوان التذكرة مطلوب" });
  
  const ticket = { id: Date.now(), subject, username: username || "عضو", status: "مفتوحة" };
...
app.post("/api/public/applications/create", (req, res) => {
  const { details, username } = req.body;
  if (!details) return res.status(400).json({ error: "تفاصيل التقديم مطلوبة" });
  
  const appItem = { id: Date.now(), details, username: username || "عضو", status: "قيد المراجعة" };
  applicationsList.push(appItem);
  res.json({ ok: true, application: appItem });
});
